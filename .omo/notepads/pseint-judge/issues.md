...
## Todo 42 — Playwright E2E journeys (2026-09-06)

- **`npm run test:e2e` had to be re-aimed at the repo-root `package.json`**: my first instinct was to add the script to `web/frontend/package.json` (where the SPA already lives) and reference the config via `--config ../../playwright.config.ts`. That works for the CLI invocation but the require resolver still misses `@playwright/test` because Node walks up from the config file, not from the cwd. Resolved by creating a separate `package.json` at the repo root with the `@playwright/test` devDependency and the `test:e2e` script there. The two package.json files don't conflict (frontend's is `private: true` for the SPA bundle; root's is `private: true` for tooling).
- **`npm init -y` was smarter than expected**: the auto-generated description pulled in the prose from `README.md`'s opening paragraph. Worth knowing for future projects — `npm init -y` reads the README and seeds the description field if one isn't already in package.json.
- **`npm uninstall` does NOT restore the previous state of `package.json` exactly**: removing `@playwright/test` from `web/frontend/` after I'd added it left a stale entry in the lockfile until the next `npm install` ran. Cosmetic; the manifest was correct. The lesson is to avoid adding a dep to web/frontend/ if you're going to remove it 30 seconds later.
- **Docker daemon unavailable in this sandbox** (same caveat as todos 36 / 37 / 40 / 41): the live `npx playwright test e2e` against the running compose stack cannot be exercised here. The structural shape of every spec is pinned by `npx playwright test --list` → 8 tests in 6 files, both projects enumerated. The live gate runs on the developer's laptop with `docker compose up -d` + `python scripts/seed.py` + `npm run test:e2e`.
- **Spec files in `e2e/` are not picked up by the `web/frontend/tsconfig.json`**: the frontend tsconfig `include` is `["src/**", "test/**", "vitest.config.ts", "vite.config.ts"]`. Playwright uses its own esbuild loader (via the @playwright/test internal bin) so the spec files compile fine — `npx playwright test --list` resolved all 6 files without errors. If a follow-up wants `tsc --noEmit` coverage over the spec files, a separate `e2e/tsconfig.json` is the right move.
- **Browser binaries not installed in this sandbox**: `npx playwright install --with-deps chromium` would download ~150MB of browser + Linux libs; documented in `e2e/RUN.md` as a prerequisite. Without the install, `playwright test --list` works (the listing is metadata-only) but the actual `test` runs would fail at browser launch. The user's environment will have the binaries already if they've done previous playwright work; otherwise the one-time install is fast.

## Bug #2 — Student layout nav stuck on the left (2026-09-06)

- **No product bugs in the fix** — 12 new tests + 107 pre-existing = 119/119 green, tsc clean, vite build clean. The fix is a 2-file CSS+TSX change with one new test file (mocks the existing todo 27 mock-fetch pattern). Container rebuilt and volume-synced; the served bundle now contains the new CSS rules (verified via curl).
- **Pre-existing duplication to flag (not in scope for bug #2)**: `StudentLayout.tsx` and `AdminLayout.tsx` both implement "header + nav + user badge + logout" but with different CSS class names and slightly different DOM shapes. A shared `<AppHeader nav={...} user={user} onLogout={...} />` component would consolidate the bug surface (today, a CSS change to the nav row has to be made twice — once for admin, once for student). Not blocking for this bug; documented as a todo-43 follow-up.

## Bug 3 — markdown math rendering (2026-09-06)

- **`react-markdown@9` removed the `inline` prop from the `code` component override**: first-cut code destructured `{inline, className, children, ...props}`. `tsc` rejected with `TS2339: Property 'inline' does not exist on type 'ClassAttributes<HTMLElement> & HTMLAttributes<HTMLElement> & ExtraProps'`. The ComponentProps<Key> typing in v9 doesn't surface the v8 inline flag. Resolved by inferring block-vs-inline from the `className` regex (`language-pseint` = block). Documented in learnings.
- **First-cut entity decoder double-decoded `&amp;gt;`**: my first decoder mapped `&amp;` to `&`, which combined with CommonMark's own entity decoding produced `&amp;gt;` → `&gt;` → `>`. The test "does not double-decode escaped entities" failed with `expected 'literal > text' to contain '&gt;'`. Fix: drop `&amp;` from the decoder's allow-list; let CommonMark handle it for prose text and accept that `&amp;` inside math won't decode (KaTeX doesn't need it — `\geq` is the LaTeX form, not `&geq;`).
- **Block math needs newlines**: first test asserted `expect(container.querySelector(".katex-display")).not.toBeNull()` on `$$F(0) = 0, F(1) = 1$$` — failed because `remark-math` parsed the single-line form as inline math (wrapped in `<p>`, no `katex-display`). Switched to `$$\nF(0) = 0, F(1) = 1\n$$`. Same lesson documented in learnings.
- **`>=` in LaTeX is two characters, not `\geq`**: first inline-math test asserted `expect(text).toContain("≥")` for source `$n >= 0$` — KaTeX returned `n >= 0` (with relation spacing, literal `>=`). Resolved by switching the seed and tests to `$n \geq 0$`. Lesson: when authoring math, use the LaTeX command names (`\geq`, `\leq`, `\neq`) — Unicode in source is not the same as the typeset output.
- **Bundle size warning post-install**: `npm run build` now warns about the 1.1MB JS chunk (was ~200KB pre-fix). KaTeX + react-markdown + unified add ~900KB; this is expected for a math-capable renderer. Code-splitting would help; not done in this commit because the cost (extra lazy-import hook) outweighs the benefit (4 problems, low-traffic SPA).
- **No issues in product code**: all 130 frontend tests pass after the fix (was 119 pre-fix; 11 new markdown tests). TypeScript clean. Build succeeds. KaTeX CSS imported once at the app root; fonts are bundled as separate assets and served by Caddy from `/srv/assets/`.

## [2026-09-07] Bug #1 — pre-existing infrastructure issues that surfaced during the fix

- **Image-tag collision** between `infra/Dockerfile.worker` (engine sandbox) and `infra/Dockerfile.rqworker` (RQ pool) — both tagged `pseint-judge-worker:latest`. `docker compose build worker` overwrites the engine image, and the wrapper then spawns the rqworker image (which tries to start a Redis listener, not run an engine). Renamed the engine image to `pseint-judge-engine:latest`; updated `DEFAULT_IMAGE` in the wrapper and `$WORKER_IMAGE` in compose.
- **Missing docker CLI in the rqworker image** (`python:3.11-slim` doesn't ship it). Pre-existing — the wrapper calls `subprocess.run(['docker', 'run', …])` but there's nothing to exec. Added `infra/scripts/install-docker-cli.sh` (downloads the static CLI via `urllib` since slim lacks curl/wget) and wired into `Dockerfile.rqworker`.
- **Worker container user (UID 65534) not in the host's docker group** — gets `permission denied` on `/var/run/docker.sock` even with the docker CLI installed. Pre-existing. Added `group_add: ["${DOCKER_GID:-957}"]` to the compose worker service and `DOCKER_GID=957` to `infra/.env` (Debian/Ubuntu's docker group gid).
- **`--stop-timeout 8` is NOT a wall budget** — it's the grace period AFTER SIGTERM is sent. The engine inside a container running an infinite loop never gets SIGTERM (no signal source), so `--stop-timeout` doesn't fire. The wrapper must enforce the wall budget via `subprocess.run(timeout=N)` itself; on `TimeoutExpired` it synthesises a `SandboxResult` with `report.error.code == "ERR_STEP_LIMIT"` so the worker classifies as TLE. The task brief docs ("--stop-timeout 8 kills the container at 8s wall") are technically wrong on this point.
- **`Dockerfile.worker` build bug**: `pip cache purge` fails when `PIP_NO_CACHE_DIR=1` is set (`pip cache commands can not function since cache is disabled`). The chained `&& rm -rf` never runs. Pre-existing. Removed the redundant `pip cache purge` line.
- **Sandbox bind-mount source paths are HOST paths** (the docker daemon runs on the host, not in the worker container). Writing a tempfile to the worker container's `/tmp` and then `-v <worker_tmp_path>:/tmp/input.txt:ro` doesn't work — the daemon can't see the worker container's filesystem. Switched to a stdin sentinel split (`source + b"\n===PSEINT-INPUT===\n" + input`) to avoid filesystem coordination entirely.

## [2026-09-07] Bug #2 — parser dispatched `Cadena s` as a SubProcCall

- **Root cause**: `_parse_identifier_statement` in `engine/src/pseint_engine/parser.py` only branched on three next-token shapes (`=`, `[`, or `(`). When a statement's first token was an `IDENTIFIER` whose lowercase name matched a type keyword (e.g. `Cadena`), the fall-through returned `SubProcCall("Cadena", [])`, which the evaluator then refused with `ERR_TYPE "subproceso no definido: Cadena"`.
- **The `_is_type_name` helper already existed** (around `parser.py:240`) and was only used inside `_expect_type_name`. The fix was to call it from the statement dispatcher too: a single new branch at the top of `_parse_statement` that catches the type-name-first-token case BEFORE `_parse_identifier_statement` sees it.
- **Impact**: every PseInt program using the `Cadena s` / `Entero i` / `Real x <- 3.14` typed-declaration syntax produced RE at parse-or-execute time. This is the most common declaration form in beginner PseInt code, so the bug blocked the entire student-side flow that needs typed locals.
- **Fix shape**:
  - `Assignment` AST node extended with `value: Expr | None = None` and `type_name: str | None = None` (frozen dataclass — `target` also defaulted to `Identifier(0,0)` via `field(default_factory=...)` to satisfy the "no defaults before non-defaults" rule).
  - New `_parse_typed_declaration` method consumes the type-name token, expects the variable name, optionally consumes `<- expr` (or `= expr`), and returns an `Assignment`.
  - Evaluator's `_exec_assignment` distinguishes three forms:
    - `value is None and type_name is not None`: declare + set zero value (via new `_zero_value` helper).
    - `type_name is not None` with init: coerce the eval'd value to the declared type and set.
    - Default (existing path): eval + implicit typing.
- **Evidence**: `.omo/evidence/parser-type-name-fix.txt` (pre-fix run_id 71 → post-fix run_id 72, AC for the user's exact program).

## [2026-09-07] Bug #2 — step budget never reached the engine (TLE bug)

- **Symptom**: An infinite loop in a user's program (Mientras i < 10000) returned `summary_verdict=AC` with `steps=40009`, instead of TLE. Wall time was ~3s per case (docker spawn overhead, not the engine loop time).
- **Root cause**: The wrapper (`scripts/run_sandboxed.py`) built the docker argv WITHOUT any `PIPELINE_MAX_STEPS` env flag. The entrypoint (`infra/entrypoint.py`) had no `os.environ` lookup for a step budget and passed nothing to the engine CLI. The engine CLI's `step_budget` default is `None`, so `_check_step_budget` was never triggered.
- **Why the original tests didn't catch it**: `test_process_run_infinite_loop_yields_tle` in `tests/test_worker.py` directly mocked the SandboxResult's report to carry `error.code=ERR_STEP_LIMIT`. This bypassed the entire wrapper→entrypoint→engine-CLI chain and only verified that the worker's `_verdict_from_sandbox` correctly maps a pre-existing ERR_STEP_LIMIT to the TLE verdict string. The plumbing that PRODUCES that error.code was untested end-to-end.
- **Fix shape** (4 files, no engine changes):
  - `scripts/run_sandboxed.py`: `max_steps` kwarg → `-e PIPELINE_MAX_STEPS=<N>` in the docker argv; matching `--max-steps` CLI flag for manual debugging.
  - `infra/entrypoint.py`: extracted `_pipeline_max_steps_argv(env)` and `build_cmd(extra_args, env)` as pure functions; reads `PIPELINE_MAX_STEPS` and prepends `--max-steps N` to the engine CLI argv.
  - `infra/worker/worker.py`: new `_step_budget_for_case(problem, tc_input)` uses `pseint_judge.complexity.expected_steps` (SPEC §(k) NORMATIVE coefficient table) + the `2*expected + 1000` formula; per-case loop calls `sandbox_runner(..., max_steps=budget)`.
  - 43 new tests across `tests/test_entrypoint.py` (new file), `tests/test_sandboxed.py`, `tests/test_worker.py`, and `engine/tests/test_cli.py`.
- **Engine image build gotcha**: the worker process loads modules from the live source tree via volume mount (fast iteration), but the engine sandbox image (`pseint-judge-engine:latest`, built by `infra/Dockerfile.worker`) is baked at image-build time and caches. After modifying the entrypoint, the engine image MUST be rebuilt (`docker build -f infra/Dockerfile.worker -t pseint-judge-engine:latest .`) — `docker compose build worker` only rebuilds the rqworker image. Forgetting this left the engine image with the old entrypoint and the bug looked "still present".
- **The wrapper's `--stop-timeout 8` is NOT what catches infinite loops**: it's the post-SIGTERM grace period, but the engine inside the container never receives SIGTERM (no signal source). The wrapper's `subprocess.run(timeout=DEFAULT_WALL_TIMEOUT_S)` synthesizes ERR_STEP_LIMIT on TimeoutExpired — this is the M9 wall-time backstop. After the fix, the engine's `_check_step_budget` (step-based) fires FIRST, so the verdict comes from the engine report, not the wall-time fallback.
- **SPEC §(k) hard TLE cap formula is normative**: `2*expected + 1000` for non-`other` complexities. HolaMundo (O(1), step_budget=50) → 2*50 + 1000 = 1100. The user's 10000-iteration loop crosses 1100 steps at iteration ~367 → ERR_STEP_LIMIT at steps=1101 (strict-greater per todo 8 invariant).
- **Wall time after the fix is ~1.7s per case, NOT 8s**: 1.7s is the docker-spawn + engine-startup overhead, not loop execution. Before the fix the engine ran the full ~10000 iterations → ~2.5s per case. After the fix the engine stops at step 1101 → ~1.7s per case (slightly faster because the engine doesn't burn cycles after the budget check fires). The brief said "under 500ms" — that's NOT achievable with the current docker overhead, but the budget mechanism is doing its job: ERR_STEP_LIMIT comes from the engine, not from the wall-time fallback.

## [2026-09-07] Forums tab — subagent route never deployed

- **`GET /api/threads` returned 404 although the source was correct**: `web/api/src/pseint_api/routes/forums.py:259` defines `@router.get("/threads", ...)` and `main.py:337` includes it — the defect was purely that the api **container image predated the route** (api was "Up 12 hours", `web/api/Dockerfile` bakes source at build time, no volume, no `--reload`). Rebuilding with `docker compose build api --no-cache` + `up -d --force-recreate api` brought `/api/threads` live (→ 200 `{items,page,size,total}`). Lesson: a subagent that ships backend code without rebuilding the image leaves the running stack stale; always build+recreate before declaring a route "not registered".
